...
$DcraftFusionChartVersion = "1.5.0"
$FusionCdcChartVersion = "1.5.0"
...
if ($exists -ne "1") {
  ...
  if ($LASTEXITCODE -ne 0) { throw "CREATE DATABASE fusion_cdc_metadata failed (exit $LASTEXITCODE) â€” aborting deploy" }
}
...
foreach ($rel in @("fusion", "fusion-cdc")) {
  ...
  if ($st.info.status -in @("failed", "pending-install", "pending-upgrade", "pending-rollback", "uninstalling")) {
    ...
    if ($LASTEXITCODE -ne 0) {
      Write-Host "WARN: helm uninstall $rel failed (exit $LASTEXITCODE) â€” manually clearing release secrets"
      kubectl -n dcraft-local delete secret -l name=$rel,owner=helm --ignore-not-found
      if ($LASTEXITCODE -ne 0) {
        throw "Failed to clear stuck Helm release $rel (uninstall + secret delete both failed, exit $LASTEXITCODE) â€” aborting deploy"
      }
...
if ($LASTEXITCODE -ne 0) {
  throw "kubectl patch deploy fusion-cdc-control-plane failed (exit $LASTEXITCODE) â€” aborting deploy"
}
kubectl -n dcraft-local rollout status deploy/fusion-cdc-control-plane --timeout=300s
if ($LASTEXITCODE -ne 0) { throw "fusion-cdc-control-plane rollout failed (exit $LASTEXITCODE) â€” aborting deploy" }
kubectl -n dcraft-local rollout status deploy/fusion-cdc-frontend --timeout=180s
if ($LASTEXITCODE -ne 0) { throw "fusion-cdc-frontend rollout failed (exit $LASTEXITCODE) â€” aborting deploy" }
kubectl -n dcraft-local rollout status deploy/fusion-dcraft-fusion-web --timeout=180s
if ($LASTEXITCODE -ne 0) { throw "fusion-dcraft-fusion-web rollout failed (exit $LASTEXITCODE) â€” aborting deploy" }
...
if (-not (Test-Path $seedSql)) {
  ...
  # missing connectors and the UI shows empty lists â€” a silent, hard-to-debug
...
Aborting deploy â€” refusing to silently fall back to the admin-only stub
...
# PowerShell's pipeline does NOT send a raw byte stream â€” PowerShell formats
# each byte object as text, corrupting the SQL (and mangling the â†’ arrow,
...
if ([string]::IsNullOrWhiteSpace($pgPod)) {
  throw "Could not find a postgres pod (label app=postgres) to seed into â€” aborting."
}
...
if ($LASTEXITCODE -ne 0) { Pop-Location; throw "kubectl cp seed-admin.sql failed (exit $LASTEXITCODE) â€” aborting deploy" }
Pop-Location
...
if ($LASTEXITCODE -ne 0) {
  throw "Seed failed â€” psql exited $LASTEXITCODE. The CDC metadata DB was NOT seeded. Aborting deploy. Inspect the pod logs and the seed SQL at $seedSql."
}
...
Fusion login (kernel seed â€” separate from CDC):
